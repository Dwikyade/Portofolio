...
const DiscordIcon = ({ size = 24, className = "" }: { size?: number | string, className?: string }) => (
...
const SOCIAL_LINKS = [
  { label: "GitHub", href: "https://github.com/dwikyade", Icon: Github },
  { label: "Discord", href: "https://discord.com/channels/1410178734559858760", Icon: DiscordIcon },
  { label: "LinkedIn", href: "https://www.linkedin.com/in/dwiky-ade-putra-zakaria-142ab13b8/", Icon: Linkedin },
  { label: "Instagram", href: "https://instagram.com/dwky.a", Icon: Instagram },
  ...
  return (
    <span
      ...
      style={{
        ...
        transition: "opacity 0.35s cubic-bezier(0.16,1,0.3,1), transform 0.35s cubic-bezier(0.16,1,0.3,1), color 0.4s ease",
      }}
    >
      {CYCLING_WORDS[index]}
    </span>
...
const MarqueeRow = ({ isDark }: { isDark: boolean }) => {
  ...
  return (
    <div
      className={`overflow-hidden border-b transition-colors duration-500 ${isDark ? "border-white/[0.05]" : "border-zinc-200"
        }`}
    >
      <div className="flex w-max animate-marquee py-3.5">
        {tripled.map((item, i) => (
          <span
            key={i}
            className={`flex items-center gap-3.5 pr-12 text-[12px] font-medium tracking-[0.08em] uppercase whitespace-nowrap transition-colors duration-500 ${isDark ? "text-white/20" : "text-zinc-400"
              }`}
          >
            <span
              className={`w-[3px] h-[3px] rounded-full flex-shrink-0 ${isDark ? "bg-white/20" : "bg-zinc-300"
                }`}
            />
            {item}
          </span>
...
export const CinematicFooter = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      ctx = gsap.context(() => {
        ...
        tl.fromTo(headingRef.current, { opacity: 0, y: 50 }, { opacity: 1, y: 0, duration: 1.1, ease: "power3.out" })
          .fromTo(buttonsRef.current, { opacity: 0, y: 35 }, { opacity: 1, y: 0, duration: 0.9, ease: "power3.out" }, "-=0.6")
          .fromTo(bottomRef.current, { opacity: 0 }, { opacity: 1, duration: 0.7, ease: "power2.out" }, "-=0.3");
      }, outerRef);
    };
  ...
  const primaryIcon = isDark ? "text-white/55 group-hover:text-white" : "text-zinc-500 group-hover:text-zinc-900";
  ...
  return (
    ...
    >
      ...
      <div className={`absolute inset-0 transition-colors duration-500 ${surface}`} />
      ...
      >
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-5 sm:px-8 py-14 md:py-20 pb-56 md:pb-64 relative z-10">
          ...
          >
            ...
            <br />
            <CyclingWord colorClass={`transition-colors duration-500 ${heading2}`} />
          </h2>

          {/* Divider */}
          <div className={`w-10 h-px mb-14 transition-colors duration-500 ${divider}`} />
          ...
          >
            ...
            <MagneticWrapper>
              <a
                href="mailto:dwiky.adeputra@gmail.com"
              ...
              >
                <Mail size={15} className={`transition-colors duration-200 ${primaryIcon}`} />
                Send Email
              </a>
            </MagneticWrapper>
            ...
            <MagneticWrapper>
              <a
                href="https://wa.me/6281217151411"
                target="_blank"
                rel="noopener noreferrer"
                className={`group flex items-center gap-3 px-8 py-4 rounded-full border font-semibold backdrop-blur-md transition-all duration-300 text-[13px] tracking-wide ${primaryBtn}`}
              >
                <MessageCircle size={15} className={`transition-colors duration-200 ${primaryIcon}`} />
                WhatsApp Me
              </a>
            </MagneticWrapper>
          </div>
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            {/* Logo */}
            <div className={`flex items-center gap-2.5 transition-colors duration-500 ${isDark ? "text-white" : "text-zinc-900"}`}>
              <Image
            ...
            {/* Nav Links */}
            <div className={`flex flex-wrap items-center gap-4 sm:gap-5 text-[13px] font-medium transition-colors duration-500 ${isDark ? "text-white/60" : "text-zinc-500"}`} style={{ fontFamily: "'Inter', sans-serif" }}>
              <button onClick={() => scrollToTop()} className="hover:text-violet-500 transition-colors">Home</button>
            ...
            <span
              ...
              style={{ fontFamily: "'Inter', sans-serif" }}
            >
              © {new Date().getFullYear()} Dwiky Ade. All rights reserved.
            </span>
          </div>
